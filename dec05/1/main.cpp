...
/* One or more Ranges are stored in each Map */
struct Range {
    u_long source_start;
    u_long dest_start;
    u_long length;
...
int main() {
    ...
    std::vector<u_long> locations({});
    for(uint j = 0; j < seeds.size(); j++) {
        locations.push_back(journey(maps, seeds.at(j)));
    }

    u_long min = locations.at(0);
    for(uint j = 0; j < locations.size(); j++) {
        if(locations.at(j) < min) { min = locations.at(j); }
    }
    std::cout << min << "\n";
}
